...
bool TryRtcp(EthernetHeaderStruct* ethernetHeader, IpHeaderStruct* ipHeader, UdpHeaderStruct* udpHeader, u_char* udpPayload)
{
	...
	std::map<int,CStdString> dictRctp;
	r = (RtcpCommonHeaderStruct*)((unsigned int *)r + ntohs(r->length) + 1);
	while(r < rtcpEnd && ((rtcpEnd - r) >= (int)sizeof(RtcpCommonHeaderStruct)))
	{
		...
		if(r->pt == 202 && count) 
		{
			...
			do
			{
				if(csrcItem == NULL)
				{
					...
					csrcItem = (RtcpSdesCsrcItem *)((char*)csrcItem + (int)csrcItem->length +2);
				}
				if(csrcItem->type == 0)
					break;
				...
				dictRctp.insert( std::make_pair(csrcItem->type,CStdString(cname)) );				
			}while((csrcItem  < (RtcpSdesCsrcItem *)rtcpThisPktEnd));
			
			
			
			/*csrcItem = (RtcpSdesCsrcItem *)((unsigned int *)r + 2);
			memcpy(cname, csrcItem->data, ((csrcItem->length > 254) ? 254 : csrcItem->length));
			logMsg.Format("RTCP1[%i] : %s", csrcItem->type, cname);
			LOG4CXX_INFO(s_rtcpPacketLog, logMsg);*/
			 
			
			
			
			/*
			while((csrcItem < (RtcpSdesCsrcItem *)rtcpThisPktEnd) && (csrcItem->type != 1) && (csrcItem->type != 0) && (csrcItem->type != 4))
			{
				...
				memcpy(cname, csrcItem->data, ((csrcItem->length > 254) ? 254 : csrcItem->length));
				logMsg.Format("RTCP1[%i]", csrcItem->type);
				LOG4CXX_INFO(s_rtcpPacketLog, logMsg);
			//#if(csrcItem < (RtcpSdesCsrcItem *)rtcpThisPktEnd && csrcItem->type == 1) Set not CNAME but PHONE
			if(csrcItem < (RtcpSdesCsrcItem *)rtcpThisPktEnd && csrcItem->type == 4)
			{
				break;
			}*/
	...
	std::map<int,CStdString>::iterator pair;
RtcpSrcDescriptionPacketInfoRef info(new RtcpSrcDescriptionPacketInfo());	
	...
	for(pair = dictRctp.begin(); pair != dictRctp.end(); pair++)
	{
		if(pair->first == 1)
		{
						
						strcpy(cname, pair->second.c_str());

			
						

						if(pair->second.length() == 0 || ACE_OS::strncasecmp(cname, "ext", ((3 > pair->second.length()) ? pair->second.length() : 3)))
						{
	...
	VoIpSessionsSingleton::instance()->ReportRtcpSrcDescriptionInTags(info,dictRctp);
...
void DetectUsefulTcpPacket(EthernetHeaderStruct* ethernetHeader, IpHeaderStruct* ipHeader, int ipHeaderLength, u_char* ipPacketEnd)
{
	...
	if(ntohs(tcpHeader->source) == DLLCONFIG.m_h225TcpPort || ntohs(tcpHeader->dest) == DLLCONFIG.m_h225TcpPort)
	{
	
		detectedUsefulPacket = true;
		MutexSentinel mutexSentinel(s_mutex);		
		ScanAllH225Messages(ethernetHeader, ipHeader, tcpHeader, ipPacketEnd);
	}
	else if(ntohs(tcpHeader->source) == DLLCONFIG.m_skinnyTcpPort || ntohs(tcpHeader->dest) == DLLCONFIG.m_skinnyTcpPort)
	{
		detectedUsefulPacket = true;
		MutexSentinel mutexSentinel(s_mutex);		// serialize access for competing pcap threads
