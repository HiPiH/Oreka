...
void VoIpSession::Stop()
{
	...
	std::list<CStdString>::iterator ids;
	int x = 0;
	for(ids = m_callsIds.begin(); ids != m_callsIds.end(); ids++)
	{
			CStdString element = *ids;
			key.Format("CallId%i",x);
			event.reset(new CaptureEvent());
			event->m_type = CaptureEvent::EtKeyValue;
			event->m_key = key;
			event->m_value = element;
...
void VoIpSession::ReportRtcpSrcDescriptionInTags(std::map<int,CStdString>& tags)
{	
	if(!m_started) return;
    CaptureEventRef event(new CaptureEvent());
	CStdString key, value,logMsg;
	std::map<int,CStdString>::iterator pair;
	for(pair = tags.begin(); pair != tags.end(); pair++)
	{
		value = pair->second;
		
		if(pair->first == 4 && ACE_OS::strchr(value, ':'))
		{
			m_callsIds.push_back(value);
		}else{
			key.Format("rtcp_%i",pair->first);
			event.reset(new CaptureEvent());
			event->m_type = CaptureEvent::EtKeyValue;
			event->m_key = key;
			event->m_value = value;
...
void VoIpSessions::ReportRtcpSrcDescriptionInTags(RtcpSrcDescriptionPacketInfoRef& rtcpInfo,std::map<int,CStdString>& tags)
{
	VoIpSessionRef session;
	...
	if(session.get() != NULL)
	{
		session->ReportRtcpSrcDescriptionInTags(tags);
		return;
	}
