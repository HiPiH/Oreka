		...
		try
		{
			...
			else
			{
				...
				else
				{
					...
					newFileName = fileNameWoExt + CONFIG.m_extentionOut;		//This is final name of audiofile = convetional orkaudio name + original name + .gsm.wav ext
					...
					CStdString finalExt;// = fileNameWoExt + CONFIG.m_extentionOut;
					finalExt.Format(".%s",CONFIG.m_extentionOut);	

					//Not really extension, just make it easier to append to the end of file name
					//CStdString finalExt = fileNameWoExt + CONFIG.m_extentionOut;
					//logMsg.Format("command newExt '%s'",finalExt);
                                        //LOG4CXX_ERROR(LOG.batchProcessingLog, logMsg);
